...
let palabras = ["ALURA", "ALTA", "ABILITY", "ACCOUNT", "ADVANCE", "AFTERNOON", "AGREE", "ALMOST", "ANALYSIS", "ANYTHING", "BALANCE", "BEAUTIFUL", "BELIEVE", "BUSINESS", "CHALLENGE", "COMPUTER", "CONTAIN", "CULTURE", "CUSTOMER", "DAUGHTER", "DEGREE", "DEVELOPMENT", "DOCUMENT", "DRAW", "EDUCATION", "EMPLOYEE", "ENVIRONMENT", "FAMILY", "FINISH", "FOCUS", "FORWARD", "FUNCTION", "FOUNDATION", "GENERATION", "GUESS", "HEALTH", "HOLIDAY", "IDENTIFY", "IMPROVE", "INTERVIEW", "KEEP", "LANGUAGE", "LEADER", "LEVEL", "LOSE", "MAGAZINE", "MANAGER", "MEAN", "MEMORY", "NETWORK", "OBJECT", "PARTY", "POPULAR", "POSSIBILITY", "PRODUCTO", "ORACLE"];
let secreto = "";
let ltrCorrecta = [];
let ltrIncorrecta = [];
...
function dibujarLetras() {
  ...
  for (let ltr of ltrIncorrecta) {
    ...
    if (conteo > 5) {
      ...
      dibujarHombre(88 + 110, 37, "#000c0c", conteo);
    }
  }
  if (conteo == 0) {
    finalJuego(false);
...
function newWord() {
  ...
  if (cantLetras < 1 || cantLetras > 10) {
    ...
    if (palabras.indexOf(palabra) < 0) {
      ...
      alert("The word " + palabra + " was add to the list!");
      nuevaPalabra.value = "";
      nuevaPalabra.focus();
    } else {
      alert("The word " + palabra + " was already added!");
    }
...
function revelar() {
  if (secreto != "") {
    alert("The secret word is " + secreto + ".");
  } else {
    alert("Shows the secret word when the game starts.");
  }
}
